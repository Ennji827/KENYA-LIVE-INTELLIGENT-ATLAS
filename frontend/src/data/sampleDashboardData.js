...
export const sampleCounties = kenyaCountyNames.map((name, index) => {
  const countyNumber = index + 1;
  const countyCode = String(countyNumber).padStart(3, "0");
  return {
    name,
...
export const nationalSummary = {
  ...
  grasslandPct: null,
  reportsReady: 0,
  countiesTracked: sampleCounties.length,
...
export function getCountyByName(name) {
  if (!name) return null;
  return sampleCounties.find((county) => county.name === name) || null;
}
